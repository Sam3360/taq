...
- **Dependency resolution**: a straightforward resolver built on `packaging`
  ...
  each package it picks the newest version that satisfies every specifier
  seen so far. This is **not** a full backtracking/SAT solver like modern
  pip — see [Limitations](#limitations).
...
- **Wheels only.** If a project only ships an sdist (no wheel for your
  ...
  scope for a v2, not something to bolt on quickly.
